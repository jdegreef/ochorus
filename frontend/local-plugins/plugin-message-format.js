/*
 * Vendored copy of @inlang/plugin-message-format@4.4.0 (dist/index.js).
 * The bundle below this header is unmodified.
 *
 * Source:  https://www.npmjs.com/package/@inlang/plugin-message-format
 * License: MIT — Copyright (c) 2024 Opral US Inc.
 *          Full text in ./plugin-message-format.LICENSE
 *
 * Why vendored: the inlang SDK loads any settings `modules` entry that does NOT
 * start with "http" from a local path (importLocalPlugins). settings.json used
 * to point at a jsdelivr CDN URL, which is fetched at compile time and fails
 * offline or behind restricted egress — breaking `npm run paraglide`/`check`/
 * `build`. Referencing this committed file removes that build-time network call.
 *
 * To update: `npm pack @inlang/plugin-message-format`, then replace everything
 * below this header with the new dist/index.js and bump the version note above.
 */
