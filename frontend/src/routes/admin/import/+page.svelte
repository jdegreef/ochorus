<script lang="ts">
	...
	// The language list is an admin endpoint, so loading it is also the gate —
	// the only page under /admin that had none. Its failure used to be caught and
	// discarded, which left a non-admin looking at the whole upload form and
	// finding out they weren't allowed only when Publish came back with a raw
	// API error.
	const importLanguages = adminResource(
		listImportLanguages,
		"Couldn't load the import form."
	);

	// Public, and deliberately outside the gate: if it fails the author picker
	// just starts empty, which is recoverable — "add a new author" is right there.
	...
	onMount(async () => {
		authors = await listAuthors().catch(() => authors);
	});
...
<div class="mx-auto max-w-2xl px-5 py-8">
	...
	<AdminGate resource={importLanguages} errorTitle="Couldn't load the import form">
