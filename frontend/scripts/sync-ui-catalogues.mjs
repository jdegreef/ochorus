 ...
 * the same numbers on every CI run and fails if this file disagrees. A stale
...
export function buildSummary(messagesDir = MESSAGES) {
	...
	/** @type {Record<string, { present: number; missing: string[] }>} */
	const out = {};
	for (const locale of locales) {
		...
		const missing = base.filter((k) => !have.has(k)).sort();
		out[locale] = { present: base.length - missing.length, missing };
	}

	return {
		_comment:
			'GENERATED by frontend/scripts/sync-ui-catalogues.mjs — do not edit by hand. ' +
			'Run `npm run sync:catalogues` in frontend/ after changing messages/*.json.',
		base_locale: BASE_LOCALE,
		base_keys: base.length,
		locales: out
	};
}
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
	...
	if (process.argv.includes('--check')) {
		...
		const line = Object.entries(s.locales)
			.map(([l, v]) => `${l} ${v.present}/${s.base_keys}`)
			.join(' · ');
