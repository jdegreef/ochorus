...
changed. So an existing painting is left as committed — PROVIDED it was cut
from this entry: ``library/art_sources.py`` records each painting's
``crop_recipe`` (object and focus) when it is drawn, and a painting whose entry
has since moved to another artwork or another focus is redrawn. Its licence is
re-checked and its rows repointed on every run either way. ``--force`` redraws
regardless.
...
SOURCES_FILE = settings.BASE_DIR / "library" / "art_sources.py"


def write_art_sources(table: dict[str, str]) -> None:
    """Rewrite ``library/art_sources.py`` — what each painting was cut from."""
    body = "\n".join(f'    "{slug}": "{recipe}",' for slug, recipe in sorted(table.items()))
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        if drawn:
            write_art_sources({**ART_SOURCES, **drawn})
