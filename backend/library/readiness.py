...
#: in production). It lists each locale's MISSING keys and carries no counts, so
#: it doesn't change (or merge-conflict) every time a UI string is added.
CATALOGUE_SUMMARY = Path(__file__).resolve().parent / "data" / "ui_catalogues.json"


def _ui_gap(code: str) -> tuple[int | None, int | None] | None:
    """(missing, total) interface strings for ``code``, or None if unknowable.

    ``missing`` is None when the locale has no catalogue at all. ``total`` is
    None when only the summary could be read — it records gaps, not sizes.
    ...
    try:
        ...
        return (None if row is None else len(row["missing"])), None
...
def _ui_check(lang: Language) -> Check:
    ...
    missing, total = gap
    if missing == 0:
        detail = f"All {total} translated." if total is not None else "All translated."
        return Check("ui", "Interface strings", PASS, detail, total, total)
    ...
    return Check(
        ...
        None if total is None else total - missing,
        total,
        forceable=False,
    )
