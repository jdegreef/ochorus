...
for "translated". Placeholders fail the bar forceably; a missing key or
catalogue fails it unforceably (see ``_ui_check``).
...
class Check:
    ...
    # See ``_ui_check``'s missing-key branch.
    forceable: bool = True
...
def _ui_check(lang: Language) -> Check:
    ...
    if missing == 0:
        ...
        # fail the bar — some, like the feedback form, do reach readers in
        # English — but FORCEABLY: the build accepts declared debt, so launching
        # past it is a judgement ("good enough to open"), not a red deploy.
        return Check(
            "ui",
            "Interface strings",
            FAIL,
