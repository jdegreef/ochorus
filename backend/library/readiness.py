...
#: (missing keys per locale — see its header for why there are no counts).
...
#: ``_ui_missing``'s answer for a locale with no catalogue at all.
NO_CATALOGUE = -1


def _ui_missing(code: str) -> int | None:
    """How many interface strings ``code`` lacks, ``NO_CATALOGUE``, or None if
    unknowable.
    ...
    if msgs is not None:
        try:
            ...
            if not target.exists():
                return NO_CATALOGUE
            base, have = (
                {k for k in json.loads(f.read_text("utf-8")) if k[:1] != "$"}
                for f in (msgs / "en.json", target)
            )
            return len(base - have)
