"""Create three biography-only authors on already-seeded databases.

Augustine of Hippo, Lemuel Haynes, and William Law join the Biographies
section with a short ``bio`` card summary and a long-form ``bio_html``. They
have no books or sermons yet, so none of the content seeds reach them:
``seed_books``/``seed_sermons`` create authors only as a side effect of
importing a work, and ``seed_if_empty`` populates a fresh database only.

Follows the pattern of ``0026_add_biography_authors`` (which used the old
single-file fixture): read the committed fixture — the source of truth — and
create the missing rows on the live DB.
...
class Migration(migrations.Migration):
    dependencies = [
        ("library", "0052_site_bio_expansions"),
    ]
