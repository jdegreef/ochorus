...
``library/language_seed.py`` without this migration hard-coding them.

Generated rather than hand-written, after a hand-written version carried a
``default=""`` the model did not declare and failed ``makemigrations --check``
with a phantom follow-up migration. The check is the point: state that only
looks equivalent is state that drifts.
"""
